"""
Training script — upgraded to triplet loss + ROC-based threshold calibration.
"""
...
from utils.dataset import TripletDataset, PairDataset
from config import (
...
def get_train_transform():
    """Aggressive augmentation helps the model generalise to unseen conditions."""
...
# -----------------------------------------------------------------------
# Threshold calibration — ROC curve approach
# -----------------------------------------------------------------------

def calibrate_thresholds(model, val_loader, device):
    """
    Compute same/twin/different thresholds from the validation set
    using an ROC-curve-based optimal cut-point (Youden's J statistic).

    This is far more principled than a simple mean-midpoint approach.
    """
    try:
        ...
        use_sklearn = False
        print("! sklearn not installed — falling back to mean-midpoint thresholds")
    ...
    with torch.no_grad():
        for img1, img2, label in tqdm(val_loader, desc="Calibrating thresholds"):
            ...
            for d, l in zip(dists, label.numpy()):
    ...
    if use_sklearn and len(same_dist) > 0 and len(diff_dist) > 0:
        # Build binary labels: 1 = same, 0 = different
        all_dists  = np.concatenate([same_dist, diff_dist])
        all_labels = np.concatenate([np.ones(len(same_dist)), np.zeros(len(diff_dist))])

        # sklearn roc_curve expects higher score → more likely positive,
        # but distance is inverted, so negate it.
        fpr, tpr, thresholds = roc_curve(all_labels, -all_dists)
        j_scores = tpr - fpr
        best_idx  = np.argmax(j_scores)
        th_same   = float(-thresholds[best_idx])   # un-negate

        # Twin threshold: midpoint between optimal same-boundary and diff mean
        th_twin = float((th_same + np.mean(diff_dist)) / 2)
    else:
        # Fallback: mean midpoint
        th_same = float(np.mean(same_dist) + np.std(same_dist) * 0.5)
        th_twin = float((np.mean(same_dist) + np.mean(diff_dist)) / 2)
...
# -----------------------------------------------------------------------
# Validate (pair-based, for loss monitoring)
# -----------------------------------------------------------------------
...
def train():
    ...
    # Pair dataset for validation loss & threshold calibration
    ...
    val_crit  = ContrastiveLoss()   # used only for val loss monitoring
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        ...
        val_loss = validate(model, val_loader, val_crit, DEVICE)
        scheduler.step()
